"""ETL: форма N33, таблица 2300.

Назначение:
    Парсит Excel-файлы формы N33 за 2016-2024 годы и извлекает данные
    таблицы 2300 по движению контингентов больных туберкулезом.

Источник:
    data/33_8_2015-2024/*.xls(x)

Результат:
    - processed/tb_form33_2300.csv;
    - public.tb_form33_2300 при запуске с --write-db.

Запуск:
    python3 orchestrator.py run form33_2300
    python3 orchestrator.py run form33_2300 --write-db

Особенности:
    Пустые ячейки, прочерки и многоточия в числовых ячейках трактуются как 0.
"""
...
SOURCE_KEY = "form33_2300"
TABLE_CODE = "2300"
DEFAULT_TABLE_NAME = "public.tb_form33_2300"
BASE_DIR = Path(__file__).resolve().parent.parent
DEFAULT_RAW_DIR = FORM_8_33_DIR
DEFAULT_OUTPUT_DIR = PROCESSED_DIR
DEFAULT_OUTPUT_FILE = "tb_form33_2300.csv"
...
# Нормализация строк таблицы 2300.
ROW_NUMBER_TO_INDICATOR: dict[int, str] = {
    1: "Взято на учет рецидивов",
    2: "Взято на учет рецидивов, из них из III группы",
    3: "Прибыло",
    4: "Переведено в III группу",
    5: "Диагноз туберкулеза снят",
    6: "Выбыло",
    7: "Умерло от туберкулеза",
    8: "Умерло от других причин",
}

# Нормализация граф таблицы 2300.
COLUMN_NUMBER_TO_DETAIL: dict[int, str] = {
    3: "Туберкулез органов дыхания всего",
    4: "Туберкулез органов дыхания дети 0-14 лет",
    5: "Туберкулез органов дыхания подростки 15-17 лет",
    6: "Туберкулез легких всего",
    7: "Туберкулез легких дети 0-14 лет",
    8: "Туберкулез легких подростки 15-17 лет",
    9: "Другие формы туберкулеза всего",
    10: "Другие формы туберкулеза дети 0-14 лет",
    11: "Другие формы туберкулеза подростки 15-17 лет",
}
...
class ParserError(RuntimeError):
    """Ошибка парсинга или валидации данных таблицы 2300."""
...
def parse_numeric(value: Any) -> float:
    ...
    Для таблицы 2300 пустые ячейки, прочерки и многоточия трактуются как 0.
...
def find_default_input_files(raw_dir: Path) -> list[Path]:
    """Находит стандартный набор исходных Excel-файлов формы 33 для таблицы 2300."""
...
def find_sheet_name(workbook: pd.ExcelFile) -> str:
    """
    Находит лист с таблицей 2300.

    Обычно таблица лежит на листе, где в названии есть 2300:
    2200,2300,2310 или похожий вариант.
    """
    candidates = [
        sheet_name
    ...
    if not candidates:
        raise ParserError("Не найден лист, в названии которого есть 2300")
...
# -----------------------------------------------------------------------------
# Поиск границ и колонок таблицы 2300
# -----------------------------------------------------------------------------

def find_table_2300_start(df: pd.DataFrame) -> int:
    """
    Находит начало таблицы 2300.

    Таблица может быть размещена на листе вместе с блоками 2200 и 2310,
    поэтому ищем именно маркер «2300» и контекст движения контингентов.
    """
    for row_idx in range(len(df)):
        row_text = " ".join(clean_cell(value) for value in df.iloc[row_idx].tolist())
        normalized = normalize_text(row_text)

        if TABLE_CODE in normalized and "движ" in normalized:
            return row_idx
    ...
    for row_idx in range(len(df)):
        ...
        if "движ" in context_normalized or "контингент" in context_normalized:
            return context_start

        # Запасной вариант: если рядом с маркером 2300 найдена строка с графами
        # 1–11, считаем этот блок таблицей 2300.
        try:
    ...
    raise ParserError("Не найден заголовок таблицы 2300")


def find_marker_row(df: pd.DataFrame, table_start: int) -> int:
    """Находит строку с номерами граф 1–11."""
    ...
    for row_idx in range(table_start, search_end):
        ...
        if set(range(1, 12)).issubset(numbers):
            return row_idx

    raise ParserError("Не найдена строка с номерами граф 1–11")
...
def build_graph_column_map(df: pd.DataFrame, marker_row: int) -> dict[int, int]:
    ...
    Для таблицы 2300 берем первую последовательность граф 1–11 слева направо.
    Если рядом есть дополнительные блоки, они игнорируются.
    """
    ...
    for col_idx, value in enumerate(df.iloc[marker_row].tolist()):
        ...
        if expected_graph > 11:
            break

    missing_graphs = sorted(set(range(1, 12)) - set(graph_to_column))
    if missing_graphs:
        raise ParserError(f"Не найдены графы таблицы 2300: {missing_graphs}")
    ...
    if missing_useful_graphs:
        raise ParserError(f"Не найдены нужные графы таблицы 2300: {missing_useful_graphs}")

    return useful_graphs


def find_row_number_column(df: pd.DataFrame, marker_row: int) -> int:
    """Находит колонку, где указаны номера строк 1–8."""
    ...
    raise ParserError("Не найдена колонка с номерами строк 1–8")

# -----------------------------------------------------------------------------
# Парсинг таблицы 2300
# -----------------------------------------------------------------------------

def parse_table_2300(path: Path) -> pd.DataFrame:
    """Парсит таблицу 2300 из одного Excel-файла."""
    ...
    table_start = find_table_2300_start(df)
    ...
    for row_idx in range(marker_row + 1, search_end):
        ...
        # Это защищает от попадания строк из соседних блоков 2310.
    ...
    log.info("%s: найдено %s строк таблицы 2300", path.name, len(result))
    return result
...
def collect_data(input_files: list[Path]) -> pd.DataFrame:
    """Собирает данные таблицы 2300 из всех переданных файлов."""
    frames: list[pd.DataFrame] = []

    for path in input_files:
        log.info("Обрабатываю файл: %s", path.name)
        frames.append(parse_table_2300(path))
...
def parse_args() -> Config:
    ...
    parser = argparse.ArgumentParser(
        description="Парсер таблицы 2300 из Excel-файлов формы 33"
    )
