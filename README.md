...
- [🗺️ Схема ETL-процесса](#etl-schema)
- [🛠️ Стек](#stack)
...
<a id="etl-schema"></a>
## 🗺️ Схема ETL-процесса

ETL-процесс проекта построен как последовательный поток: исходные файлы из `data/` регистрируются и обрабатываются через единый оркестратор, затем данные нормализуются в таблицы форм, справочники и региональные таблицы. На следующем шаге SQL-слой собирает расчетные показатели и BI-витрину карты, а в Yandex DataLens передаются только финальные сущности: `public.tb_calculated_indicators` и `public.v_tb_incidence_subjects_datalens_map`.

![Схема ETL-процесса](images/tb_etl_process_schema.svg)
...
**Цель**
- Комплексный анализ ситуации по туберкулезу на федеральном и региональном уровнях а так же оценка уровня заболеваемости, качества выявления, структуры тяжёлых форм, эффективности лечения, кадрового обеспечения и нагрузки на стационарную помощь в Иркутской области.
...
- Реализован анализ ключевых показателей по туберкулезу в РФ и регионах;
- Подготовлены отдельные аналитические вкладки по заболеваемости, выявлению, профилактике, тяжести течения, смертности, лечению и ресурсам фтизиатрической службы детализированно по Иркутской области;
- В дашборд включена **аннотация** для пояснения структуры.
