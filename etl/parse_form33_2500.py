"""ETL: форма N33, таблица 2500.

Назначение:
    Парсит Excel-файлы формы N33 за 2015-2024 годы и извлекает данные
    о бактериовыделителях, состоящих на учете.

Источник:
    data/33_8_2015-2024/*.xls(x)

Результат:
    - processed/tb_form33_2500.csv;
    - public.tb_form33_2500 при запуске с --write-db.

Запуск:
    python3 orchestrator.py run form33_2500
    python3 orchestrator.py run form33_2500 --write-db
...
SOURCE_KEY = "form33_2500"
TABLE_CODE = "2500"
DEFAULT_TABLE_NAME = "public.tb_form33_2500"
BASE_DIR = Path(__file__).resolve().parent.parent
DEFAULT_RAW_DIR = FORM_8_33_DIR
DEFAULT_OUTPUT_DIR = PROCESSED_DIR
DEFAULT_OUTPUT_FILE = "tb_form33_2500.csv"
...
# Нормализация строк таблицы 2500.
ROW_NUMBER_TO_INDICATOR: dict[int, str] = {
    1: "Туберкулез органов дыхания",
    2: "Обследовано на МЛУ",
    3: "Выявлена МЛУ",
    4: "Туберкулез внелегочных локализаций",
    5: "Сельские жители",
}

# Нормализация граф таблицы 2500.
COLUMN_NUMBER_TO_DETAIL: dict[int, str] = {
    3: "Обнаружено из числа больных с впервые в жизни установленным диагнозом",
    4: (
        "Обнаружено из числа больных с впервые в жизни установленным диагнозом, "
        "из них методом посева"
    ),
    5: (
        "Обнаружено из числа больных с впервые в жизни установленным диагнозом, "
        "из них методом микроскопии"
    ),
    6: "Обнаружено из числа больных состоящих на учете в I группе",
    7: "Обнаружено из числа больных состоящих на учете в IБ группе",
    8: "Обнаружено из числа больных состоящих на учете во II группе",
    9: "Обнаружено из числа больных состоящих на учете в III группе",
    10: "Обнаружено из числа больных снятых ранее с учета",
    11: "Переведено из других учреждений больных, выделяющих МБТ",
    12: "Умерло от туберкулеза",
    13: "Умерло от других причин",
    14: "Перестало выделять МБТ",
    15: "Выбыло из района обслуживания",
    16: "Состоит на конец отчетного года",
}
...
class ParserError(RuntimeError):
    """Ошибка парсинга или валидации данных таблицы 2500."""
...
def parse_numeric(value: Any) -> float:
    ...
    Для таблицы 2500 пустые ячейки, прочерки и многоточия трактуются как 0.
...
def find_default_input_files(raw_dir: Path) -> list[Path]:
    """Находит стандартный набор исходных Excel-файлов формы 33 для таблицы 2500."""
...
def find_sheet_name(workbook: pd.ExcelFile) -> str:
    """
    Находит лист с таблицей 2500.

    Для 2016 года таблица лежит на листе 2500,2510,2520.
    ...
    if not candidates:
        raise ParserError("Не найден лист, в названии которого есть 2500")
...
# -----------------------------------------------------------------------------
# Поиск границ и колонок таблицы 2500
# -----------------------------------------------------------------------------

def find_table_2500_start(df: pd.DataFrame) -> int:
    """
    Находит начало таблицы 2500.

    В 2017–2024 годах заголовок обычно записан одной строкой:
    «(2500) 5. Бактериовыделители, состоящие на учете».

    В 2016 году заголовок может быть разбит на несколько строк, поэтому
    дополнительно проверяется контекст вокруг строки с маркером «2500».
    """
    for row_idx in range(len(df)):
        row_text = " ".join(clean_cell(value) for value in df.iloc[row_idx].tolist())
        normalized = normalize_text(row_text)

        if TABLE_CODE in normalized and "бактериовыдел" in normalized:
            return row_idx
    ...
    for row_idx in range(len(df)):
        ...
        if "бактериовыдел" in context_normalized:
            return context_start

        # Запасной вариант: если рядом с маркером 2500 найдена строка с графами
        # 1–16, считаем этот блок таблицей 2500.
        try:
    ...
    raise ParserError("Не найден заголовок таблицы 2500")


def find_marker_row(df: pd.DataFrame, table_start: int) -> int:
    """Находит строку с номерами граф 1–16."""
    ...
    for row_idx in range(table_start, search_end):
        ...
        if set(range(1, 17)).issubset(numbers):
            return row_idx

    raise ParserError("Не найдена строка с номерами граф 1–16")
...
def build_graph_column_map(df: pd.DataFrame, marker_row: int) -> dict[int, int]:
    ...
    Для таблицы 2500 берем первую последовательность граф 1–16 слева направо.
    Если рядом с таблицей есть дополнительные блоки 2510/2520, они игнорируются.
    """
    ...
    for col_idx, value in enumerate(df.iloc[marker_row].tolist()):
        ...
        if expected_graph > 16:
            break

    missing_graphs = sorted(set(range(1, 17)) - set(graph_to_column))
    if missing_graphs:
        raise ParserError(f"Не найдены графы таблицы 2500: {missing_graphs}")
    ...
    if missing_useful_graphs:
        raise ParserError(f"Не найдены нужные графы таблицы 2500: {missing_useful_graphs}")

    return useful_graphs


def find_row_number_column(df: pd.DataFrame, marker_row: int) -> int:
    """Находит колонку, где указаны номера строк 1–5."""
    search_end = min(marker_row + 15, len(df))
    ...
    raise ParserError("Не найдена колонка с номерами строк 1–5")

# -----------------------------------------------------------------------------
# Парсинг таблицы 2500
# -----------------------------------------------------------------------------

def parse_table_2500(path: Path) -> pd.DataFrame:
    """Парсит таблицу 2500 из одного Excel-файла."""
    ...
    table_start = find_table_2500_start(df)
    ...
    search_end = min(marker_row + 20, len(df))
    ...
    for row_idx in range(marker_row + 1, search_end):
        ...
        # Это защищает от попадания строк из соседних блоков 2510/2520.
    ...
    log.info("%s: найдено %s строк таблицы 2500", path.name, len(result))
    return result
...
def collect_data(input_files: list[Path]) -> pd.DataFrame:
    """Собирает данные таблицы 2500 из всех переданных файлов."""
    frames: list[pd.DataFrame] = []

    for path in input_files:
        log.info("Обрабатываю файл: %s", path.name)
        frames.append(parse_table_2500(path))
...
def parse_args() -> Config:
    ...
    parser = argparse.ArgumentParser(
        description="Парсер таблицы 2500 из Excel-файлов формы 33"
    )
